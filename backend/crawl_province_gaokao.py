...
class ProvinceGaokaoCrawler:
    ...
    def get_province_dir(self, province: str) -> str:
        """获取省份输出目录"""
        province_name = province.replace("卷", "")
        return os.path.join(self.output_dir, f"{province_name}卷")
...
async def main():
    """主函数"""
    async with ProvinceGaokaoCrawler() as crawler:
